...
const sbScript = `(function (vm, host, Contextify, Decontextify, Buffer) { ${sb} \n})`;
const cfScript = `(function(require, host) { ${cf} \n})`;
const exportScript = "({exports: {}})";
const PROTECTED = ['constructor', '__proto__'];
// this map lead to memory leak. But since nodejs's vm leak the script already, this
// will actually alleviate the problem.
const ScriptMap = new Map();
const VMScriptMap = new Map();

const getVmScript = (code, filename) => {
	const scriptHash = crypto.createHash('sha256').update(code, 'utf8')
		.update('***', 'utf8').update(filename || '', 'utf8').digest('hex');
	
	let globalScript = VMScriptMap.get(scriptHash);

	if(globalScript) {
		return globalScript;
	}

	let option = {displayErrors: false};
	if(filename) {
		option.filename = filename;
	}
	
	globalScript = new vm.Script(code, option)
	VMScriptMap.set(scriptHash, globalScript);
	return globalScript;
}
...
class VMScript {
	...
	constructor(code, filename) {
		this.getGlobalCode(code);
		this.filename = filename || 'vm.js';
	}

	getGlobalCode(genCode) {
		if(typeof genCode !== 'string') {
			this.code = genCode;
			return;
		}
		const genCodeHash = crypto.createHash('sha256').update(genCode, 'utf8').digest('hex');
		let globalGenCode = ScriptMap.get(genCodeHash);
		if(!globalGenCode) {
			ScriptMap.set(genCodeHash, genCode);
			globalGenCode = genCode;
		}
		this.code = globalGenCode;
	}
	...
	wrap(prefix, suffix) {
		...
		const genCode = prefix + this.code + suffix;
		this.getGlobalCode(genCode);
		this._wrapped = true;
		return this;
	}
	...
	compile() {
		...
		this._compiled = getVmScript(this.code, this.filename);
		return this;
	}
}
...
class VM extends EventEmitter {
	...
	constructor(options = {}) {
		...
		const cfCache = getVmScript(cfScript, `${__dirname}/contextify.js`);
		Reflect.defineProperty(this, '_internal', {
			value: cfCache.runInContext(this._context, {
...
class NodeVM extends EventEmitter {
	...
	constructor(options = {}) {
      ...
      sourceExtensions: options.sourceExtensions || ['js']
		...
		this._context = vm.createContext();
		const cfCache = getVmScript(cfScript, `${__dirname}/contextify.js`);
		const sbCache = getVmScript(sbScript, `${__dirname}/sandbox.js`);
		...
		const closure = sbCache.runInContext(this._context, {
	...
	run(code, filename) {
		...
		const module = vm.runInContext(exportScript, this._context, {
			displayErrors: false
		});
